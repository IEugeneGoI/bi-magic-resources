...
const Lines = ({
  ...
  const renderLines = instances.slice(0, -1).map((inst, index) => {
    ...
    const normalizedTotalNext =
      maxInstanceValue > 0 ? (totalNext / maxInstanceValue) * maxBarHeight : 0;
    ...
    const yH1 = -instanceHeights[inst];
    const yH2 =
      instanceHeights[inst] -
      (instanceHeights[nextInst] - instanceHeights[inst]);
    ...
    return (
      <g key={inst} transform={`translate(0, ${0})`}>
